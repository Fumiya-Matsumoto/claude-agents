...
auto-router (Fable) ── 必要な判断の質でタスクを 4 Tier に分類
  ...
  ※ frontier-reviewer が走るときは必ず、系列外の codex exec review (OpenAI
    Codex CLI) を並行で同時起動する（高リスクな Tier 1 で quality-reviewer が
    走るときも同様。起動点は auto-router の COMPLETION 節 /
    frontier-orchestrator / orchestrator の 3 箇所。ただしセッション 1 回・
    Codex 枠が乏しいときは見送る。未導入でも壊れません）
```
...
この不変条件は 2026-08-20 に**適用範囲を絞る形で改訂**しました（不変条件そのものを撤回したわけではありません — 新しいトリガーや新しい設定キーはいまも作っていません）。理由は実測です。ユーザーの Codex 週次枠（`planType=prolite` / `windowDurationMins=10080`）が `usedPercent=100` に到達し、直近の週次窓で系列外レビューが実際に**14 回 / 10 セッション（平均 1.4 回）**走っていたこと（`frontier-reviewer` 由来 11 件・`quality-reviewer` 由来 5 件）、うち 2026-08-20 の 3 回は枠切れで **FAILED（成果ゼロ）** に終わったこと、そして今週は**2 回発火したセッションが 4 つ**あった（＝修正後の再発火）ことが判明しました。**削るのは発火の「広さ」と「重複」であって、発火条件という設計そのものではありません。**

寄生先を**レビュアの名前ではなく HIGH-RISK SURFACE**で引き直します — 「高リスク面に触れる変更の独立レビューが走るとき」に発火します（カバー範囲は (Tier 1 ∧ 高リスク) ∨ (Tier 2 ∧ 高リスク) ∨ Tier 3 ∨ O ペインの高リスク PR）。高リスクな Tier 1 を見る `quality-reviewer` と、高リスクな Tier 2 を見る `frontier-reviewer` はどちらも引き続き寄生先です。外すのは `quality-reviewer` の**非高リスクな Tier 2 の既定レビュー**だけです。実測では `quality-reviewer` 由来が 5 件ありましたが、そのうち高リスク Tier 1 分と非高リスク Tier 2 分の内訳は**未実測**です — 今回の変更で実際に減る発火数は、最悪の場合（5 件全てが高リスク Tier 1 だった場合）ゼロ、最大でも 5 件/週です（＝最大 +5/週 分は今回の絞り込みでは戻らない可能性がある、という含みです）。加えて**1 セッションにつき最大 1 回**にします。ルーターに増える判断はゼロで、**通常の Tier 1（非高リスク）は引き続き対象外**のまま。起動点は 3 箇所のままですが、`auto-router` の 1 箇所は `quality-reviewer`（高リスク Tier 1）と `frontier-reviewer`（高リスク Tier 2）の両方の発火に対応するため、起動点とレビュアはもう 1 対 1 では対応しません:

| 起動点 | 寄生先 |
|---|---|
| `auto-router` の `COMPLETION` 節 | `quality-reviewer`（高リスクな Tier 1） / `frontier-reviewer`（高リスクな Tier 2） |
...
- **セッションにつき最大 1 回。** 修正後の再レビューでは再発火させません。今週 2 回発火したセッションが 4 つあった実測（＝ほぼ全てが「修正して再レビュー」）を踏まえた歯止めで、`bin/codex-review` が `CLAUDE_CODE_SESSION_ID` ごとのマーカーファイルで**構造的に**担保します（プロンプト規律ではなく、2 回目は exit 125 で即座に降ります）。新しい設定キー・環境変数は増やしていません
- **Codex の週次枠が乏しいときは見送ります。** `primary.usedPercent >= 70` か `rateLimitReachedType` が立っていれば起動せず、bin/codex-review 自身が `bin/codex-ratelimits`（同ディレクトリ）に乗って preflight で判定し、exit 126 で降ります。70% という線は、週次枠の**残り 3 割を常にユーザー自身の Codex 作業のために確保する**ことを買っています。ここも新しい設定キー・環境変数は増やさず、固定値としてスクリプトに置いています。取得できない・壊れている・遅い場合は「無ければ壊れる依存」にせず**何も言わずに素通り**します（`codex` CLI 自体が未導入のときの扱いと同型）
- **`model_reasoning_effort="xhigh"` はそのまま据え置きます。** 絞った後に残る発火源は、高リスク面に触れる変更と Tier 3 の初回だけです（Tier 3 は高リスク面に触れるかどうかを問わず全件残ります）。いずれも検出力が最も重要な場面です。頻度を絞ったのに検出力まで下げては本末転倒なので、この値は変更しません。1〜2 週の実測を見てから、必要ならあらためて見直します
- **「Claude 枠の節約」は根拠にしていません**（測定で反証済み。消費の 76% は主セッションで、レビュアは低頻度）。買っているのは相関除去だけです。**混同注意**: 今回絞ったのは Codex 側の週次枠であって、この Claude 側の反証とは別の話です — Claude 枠の消費実態は変わっていません
- **Claude レビュアと並行（同時起動）**。「Claude レビュアの指摘を Codex に見せない」がプロンプト規律ではなく**構造**で担保されます（レビュー時点で指摘がまだ存在しない）。同期実行の待ち時間もレビュア待ちに吸収されます
- **往復はゼロ**。指摘を採用して直せば新しい完了サイクルになり、寄生元の Claude 側レビュア（`quality-reviewer` / `frontier-reviewer`）自身の再発火規則で再度レビューが走りうる場面はありますが、系列外レビューはセッション 1 回の歯止めが優先するので追随しません。却下の記録も残しません（同じ差分を二度見ないため。同じ**型**の却下が繰り返されるなら、それは抑制対象ではなく規則側を直すシグナルなので `agents-feedback` に上げます）
- **裁き手はルーター。ただし採用・却下とも全件を原文のまま完了報告に列挙**し、各件に `採用 / 却下（理由）` を付けます。「却下する動機を持つ側が裁く」問題を、裁定権の移転ではなく**可視性**で殺す設計です（人間は上訴審、通常時の摩擦はゼロ）
- **失敗してもブロックしません**（枠切れ・CLI 未導入・実行失敗・セッション上限・枠ガード）。ただし黙って落ちず、完了報告に「系列外レビューは走らなかった（理由）」を必ず書きます
...
**未導入でも壊れません。** `codex` が無ければスクリプトは exit 127 で即座に降り、Claude レビュアだけが走ります（完了報告にその旨が明記されます）。同様に、このセッションでは既に走った（exit 125）・枠ガードで見送った（exit 126）ときも Claude レビュアだけが走ります。終了コードの全体は `bin/codex-review` 冒頭のコメントを正とします（`0`＝走った / `1`＝preflight 失敗 / `2`＝引数エラー / `125`＝セッション上限 / `126`＝枠ガード / `127`＝codex CLI 未導入）。
...
`frontier-reviewer`、および高リスクな Tier 1 を見る `quality-reviewer` による独立レビューが走る場面では、加えて**系列外の `codex exec review`（OpenAI Codex CLI・任意）が並行で走ります**（1 セッション 1 回・Codex 枠が乏しいときは見送り）。`quality-reviewer` の非高リスクな Tier 2 の既定レビューでは走りません。系列外の Codex は Claude のサブエージェントではないので frontmatter を持たず、この表には載りません（前節「系列外レビュー（Codex）」を参照）。
